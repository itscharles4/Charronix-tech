...
// Simple password hash function (bcryptjs may not be available)
async function hashPassword(password: string): Promise<string> {
  // In production, use bcryptjs. For testing, we'll use a simple hash
...
async function seedDatabase() {
  try {
    ...
    // ============================================
    // 3. CREATE STUDENTS & MEDICAL INFO
    // ============================================
    ...
    for (const student of studentData) {
      ...
      await prisma.medicalInfo.upsert({
        ...
        create: {
          ...
          allergies:
            student.bloodGroup === 'A+' ? ['Milk'] : student.bloodGroup === 'O-' ? ['Eggs'] : ['Peanuts'],
    ...
    for (const student of createdStudents) {
      // Create 22 days of February attendance
      for (let day = 1; day <= 22; day++) {
        ...
        let status = 'PRESENT';
        if (random > 90) status = 'ABSENT';
        else if (random > 80) status = 'LATE';
        else if (random > 75) status = 'LEAVE';
