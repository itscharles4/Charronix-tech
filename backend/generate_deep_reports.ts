...
async function main() {
    ...
    const teachers = await prisma.teacher.findMany({
        include: {
            subjects: true,
            classes: true,
            attendance: { take: 10 }, // Marked by
            academicGrades: { take: 10 } // Entered by
        },
    ...
    // 1. Generate db_data_report.txt (Exhaustive data dump)
    ...
    fs.writeFileSync('./db_data_report.txt', dataReport);
